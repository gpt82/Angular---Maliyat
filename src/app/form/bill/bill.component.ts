...
export class BillComponent extends GridBaseClass implements OnInit {
  isEdit = false;
  trailers: any[] = [];
  ...
  constructor(
    ...
    super(http, '/v1/api/Bill/', dialog);
    this.gridName = 'billGrid';
  ...
  onCreate(): void {
    const dialogRef = this.dialog.open(BillDialog, {
      ...
      disableClose: true,
      data: {
        Bill: new BillDto(null),
        dialogTitle: 'افزودن قبض جدید',
        isEdit: false
      }
    });
  ...
  onEdit(id: number): void {
    this.getEntity(id).subscribe(result => {
      const bill = new BillDto(result['entity']);
  ...
  onBillsListReport(): void {
    const url = '/v1/api/Report/BillsList/';
