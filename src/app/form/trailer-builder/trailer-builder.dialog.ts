...
@Component({
  selector: 'trailer-builder-dialog',
  templateUrl: 'trailer-builder.dialog.html',

})
export class TrailerBuilderDialog extends ModalBaseClass implements OnInit {
  form: FormGroup;

  constructor(
    public dialogRef: MatDialogRef<TrailerBuilderDialog>,
  ...
  ngOnInit() {
    ...
    if (this.data.isEdit === true) {
      this.form.setValue({
        name: this.data.TrailerBuilder.name
      })
    }
  }
  uniqueName(ctrl: AbstractControl): Observable<ValidationErrors | null> {
    let equalOriginal = ctrl.value == this.data.TrailerBuilder.name;
  ...
  edit(data): void {
    if (data) {
      ...
      this.http.put(this.getUrl() + data.TrailerBuilder.id,
        JSON.stringify({
          Name: this.name.value
        }), { headers: headers1 }).subscribe((result) => {
          this.dialogRef.close({ state: "successful" });
        }, (error: any) => {
          console.log("edit trailer builder");
          console.log(error);
        });
    }
  }
