...
@Component({
  selector: "trailer-cert-item-dialog",
  templateUrl: 'trailer-cert-item.dialog.html'
})
export class TrailerCertItemDialog extends ModalBaseClass implements OnInit {
  form: FormGroup;

  constructor(
    public dialogRef: MatDialogRef<TrailerCertItemDialog>,
  ...
  ngOnInit() {
    ...
    if (this.data.isEdit === true) {
      this.form.setValue({
        name: this.data.TrailerCertItem.name
      });
    }
  }
  uniqueName(ctrl: AbstractControl): Observable<ValidationErrors | null> {
    const equalOriginal = ctrl.value == this.data.TrailerCertItem.name;
  ...
  edit(data): void {
    if (data) {
      ...
      this.http
        .put(
          this.getUrl() + data.TrailerCertItem.id,
          JSON.stringify({
