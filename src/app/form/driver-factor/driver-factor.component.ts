...
import { DriverFactorDialog } from './driver-factor.dialog';
import * as moment from 'jalali-moment';

import { HttpClient } from '@angular/common/http';
import { GridBaseClass } from '../../shared/services/grid-base-class';
import { State } from '@progress/kendo-data-query';
import { DriverFactorDto } from './dtos/DriverFactorDto';
...
@Component({
  ...
  selector: 'driver-factor-component',
  templateUrl: './driver-factor.component.html',
  providers: [HttpClient]
})
export class DriverFactorComponent extends GridBaseClass implements OnInit {
  isEdit = false;
  trailers: any[] = [];
  public branches: any[];
  constructor(
    ...
    super(http, '/v1/api/DriverFactor/', dialog);
    this.gridName = 'driverFactorGrid';
  ...
  onCreate(): void {
    const dialogRef = this.dialog.open(DriverFactorDialog, {
      width: '600px',
      height: '445px',
      disableClose: true,
      data: {
        DriverFactor: new DriverFactorDto(null),
        dialogTitle: 'افزودن فاکتور جدید',
        isEdit: false
      }
    });
  ...
  onEdit(id: number): void {
    this.getEntity(id).subscribe(result => {
      const driverFactor = new DriverFactorDto(result['entity']);
  ...
  onDriverFactorsListReport(): void {
    const url = this.getUrl() + 'DriverFactorListReport/';
    ...
    this.http.get(url, { headers: headers }).subscribe(result => {
      ...
      ReportService.setReportViewModel({
        reportName: 'DriverFactorList.mrt',
        options: null,
        dataSources,
