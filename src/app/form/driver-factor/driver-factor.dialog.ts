...
@Component({
  ...
  selector: 'driver-factor-dialog',
  templateUrl: 'driver-factor.dialog.html',
  styles: [
...
export class DriverFactorDialog extends ModalBaseClass implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<DriverFactorDialog>,
  ...
  ngOnInit() {
    ...
    this.form = this.fb.group(
      {
        trailerId: this.data.DriverFactor.trailerId,
        driverId: this.data.DriverFactor.driverId,
        amount: [this.data.DriverFactor.amount,  { validators: [Validators.required], updateOn: 'change' }],
        issueDate: [moment(this.data.DriverFactor.issueDate).locale('fa'),
        Validators.required
        ],
        description: [this.data.DriverFactor.description, Validators.required]
      },
      { updateOn: 'blur' }
    );
  ...
  private loadDrivers() {
    this.drivers$ = concat(
      of([
        {
          id: this.data.DriverFactor.driverId,
          title: this.data.DriverFactor.driverName
        }
      ]),
  ...
  private loadTrailers() {
    this.trailers$ = concat(
      of([
        {
          id: this.data.DriverFactor.trailerId,
          title: this.data.DriverFactor.trailerPlaque
        }
      ]),
  ...
  onSave(): void {
    if (this.form.valid) {
      ...
      const driverFactor = JSON.stringify({
      ...
      if (this.data.isEdit === true) {
        this.http
          .put(this.getEntryPointUrl() + this.data.DriverFactor.id, driverFactor,
            { headers: header })
          .subscribe(
            result => {
            ...
            (error: any) => {
              console.log('edit driver-factor');
              console.log(error);
            }
          );
      } else {
  ...
  create(): void {
    ...
    this.http
      ...
      .subscribe(
        ...
        (error: any) => {
          console.log('create driver-factor');
          console.log(error);
        }
      );
  }
