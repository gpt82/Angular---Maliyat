...
@Component({
  selector: 'gete-loading-location-dialog',
  templateUrl: 'gete-loading-location.dialog.html',

})
export class GeteLoadingLocationDialog extends ModalBaseClass implements OnInit {
  form: FormGroup;
  constructor(
    public dialogRef: MatDialogRef<GeteLoadingLocationDialog>,
  ...
  ngOnInit() {
    ...
    if (this.data.isEdit === true) {
      this.form.setValue({
        name: this.data.LoadingLocation.name
      });
    }
  }
  uniqueName(ctrl: AbstractControl): Observable<ValidationErrors | null> {
    const equalOriginal = ctrl.value === this.data.LoadingLocation.name;
  ...
  edit(data): void {
    if (data) {
      ...
      this.http.put(this.getUrl() + data.LoadingLocation.id,
        JSON.stringify({
