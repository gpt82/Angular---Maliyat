...
@Component({
  ...
  selector: 'invalid-agenda-dialog',
  templateUrl: 'invalid-agenda.dialog.html',
  styles: [
...
export class InvalidAgendaDialog extends ModalBaseClass implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<InvalidAgendaDialog>,
  ...
  ngOnInit() {
    ...
    this.form = this.fb.group(
      {
        trailerId: this.data.InvalidAgenda.trailerId,
        driverId: this.data.InvalidAgenda.driverId,
        agendaNumber: [this.data.InvalidAgenda.agendaNumber, Validators.required],
        issueDate: [moment(this.data.InvalidAgenda.issueDate).locale('fa'),
        Validators.required
        ],
        description: [this.data.InvalidAgenda.description, Validators.required]
      },
      // { updateOn: 'blur' }
    );
  ...
  private loadDrivers() {
    this.drivers$ = concat(
      of([
        {
          id: this.data.InvalidAgenda.driverId,
          title: this.data.InvalidAgenda.driverName
        }
      ]),
  ...
  private loadTrailers() {
    this.trailers$ = concat(
      of([
        {
          id: this.data.InvalidAgenda.trailerId,
          title: this.data.InvalidAgenda.trailerPlaque
        }
      ]),
  ...
  onSave(): void {
    if (this.form.valid) {
      ...
      const invalidAgenda = JSON.stringify({
        BranchId: this.data.InvalidAgenda.branchId,
      ...
      if (this.data.isEdit === true) {
        this.http
          .put(this.getEntryPointUrl() + this.data.InvalidAgenda.id, invalidAgenda,
            { headers: header })
          .subscribe(
            result => {
            ...
            (error: any) => {
              console.log('edit invalid-agenda');
              console.log(error);
            }
          );
      } else {
  ...
  create(): void {
    ...
    this.http
      ...
      .subscribe(
        ...
        (error: any) => {
          console.log('create invalid-agenda');
          console.log(error);
        }
      );
