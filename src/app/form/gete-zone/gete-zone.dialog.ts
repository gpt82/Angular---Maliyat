...
@Component({
  selector: 'gete-zone-dialog',
  templateUrl: 'gete-zone.dialog.html',

})
export class GeteZoneDialog extends ModalBaseClass implements OnInit {
  form: FormGroup;
  constructor(
    public dialogRef: MatDialogRef<GeteZoneDialog>,
  ...
  ngOnInit() {
    ...
    if (this.data.isEdit === true) {
      this.form.setValue({
        name: this.data.GeteZone.name
      });
    }
  }
  uniqueName(ctrl: AbstractControl): Observable<ValidationErrors | null> {
    const equalOriginal = ctrl.value === this.data.GeteZone.name;
  ...
  edit(data): void {
    if (data) {
      ...
      this.http.put(this.getUrl() + data.GeteZone.id,
        JSON.stringify({
