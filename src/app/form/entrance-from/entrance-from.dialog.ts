...
@Component({
  ...
  selector: 'entrance-from-dialog',
  templateUrl: 'entrance-from.dialog.html'
})
// tslint:disable-next-line: component-class-suffix
export class EntranceFromDialog extends ModalBaseClass implements OnInit {
  form: FormGroup;
  constructor(
    public dialogRef: MatDialogRef<EntranceFromDialog>,
  ...
  ngOnInit() {
    ...
    if (this.data.isEdit === true) {
      this.form.setValue({
        name: this.data.EntranceFrom.name
      });
    }
  }
  uniqueName(ctrl: AbstractControl): Observable<ValidationErrors | null> {
    const equalOriginal = ctrl.value === this.data.EntranceFrom.name;
  ...
  edit(data): void {
    if (data) {
      ...
      this.http
        .put(
          this.getUrl() + data.EntranceFrom.id,
          JSON.stringify({
        ...
        .subscribe(
          ...
          (error: any) => {
            console.log('edit entrance from');
            console.log(error);
          }
        );
    }
    ...
    if (data) {
      ...
      this.http
        ...
        .subscribe(
          ...
          (error: any) => {
            console.log('create entrance from');
            console.log(error);
          }
        );
    }
