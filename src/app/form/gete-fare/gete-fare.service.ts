...
@Injectable()
export class GeteFareService {
  private url = '/v1/api/GeteFare';

  constructor(private http: HttpClient, private authService: AuthService) {
  }

  // getGeteFareByCode(cityId: number, contractNo: number) {
  getGeteFareByCode(request: any) {
    // const obj = {
    //   cityId: cityId,
    ...
    // );
    return this.http.get<any>(`${this.url}`); // /getGeteFare/${request.contractNo}/${request.cityId}
  }
  ...
  getAgendaGeteFare(contractNo: string, agentId: string) {
    const headers = new HttpHeaders();
    ...
    //     branchId: branchId
    // });
    // return this.http.get<any>(`${this.url}/getAgendaGeteFare`);
    //  this.http.get(`${this.url}/getAgendaGeteFare`, {observe: 'response', params}).subscribe(
    return this.http.get<any>(`${this.url}/getAgendaGeteFare`, { headers, params });
