...
export class PostComponent extends GridBaseClass implements OnInit {
  isEdit = false;
  ...
  constructor(
    ...
    super(http, '/v1/api/Post/', dialog);
    this.gridName = 'postGrid';
  ...
  onEdit(id: number): void {
    this.getEntity(id).subscribe(result => {
      const post = new PostDto(result['entity']);
  ...
  onPostsListReport(): void {
    const url = '/v1/api/Post/PostListReport/';
    ...
    this.http.get(url, { headers: headers }).subscribe(result => {
      ...
      result['details'].forEach(row => {
        detailRows.push({
          ...
          BranchTitle: row.branchTitle.replaceAll('شعبه', '')
        });
      });
