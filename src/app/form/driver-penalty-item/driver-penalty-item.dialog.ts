...
@Component({
  selector: "driver-penalty-item-dialog",
  templateUrl: 'driver-penalty-item.dialog.html'
})
export class DriverPenaltyItemDialog extends ModalBaseClass implements OnInit {
  form: FormGroup;

  constructor(
    public dialogRef: MatDialogRef<DriverPenaltyItemDialog>,
  ...
  ngOnInit() {
    ...
    if (this.data.isEdit === true) {
      this.form.setValue({
        name: this.data.DriverPenaltyItem.name
      });
    }
  }
  uniqueName(ctrl: AbstractControl): Observable<ValidationErrors | null> {
    const equalOriginal = ctrl.value == this.data.DriverPenaltyItem.name;
  ...
  edit(data): void {
    if (data) {
      ...
      this.http
        .put(
          this.getUrl() + data.DriverPenaltyItem.id,
          JSON.stringify({
